...
static inline void I2C_Slave_Mode_Clock_Configurations(const mssp_i2c_t *i2c_obj);
...
static void (*I2C_DefaultInterruptHandler_InterruptHandler)(void) = NULL;
...
void MSSP_I2C_ISR(void){
    ...
    if(I2C_DefaultInterruptHandler_InterruptHandler)
    {
        I2C_DefaultInterruptHandler_InterruptHandler();
    }
#endif
}
...
static inline void MSSP_I2C_Interrupt_Configurations(const mssp_i2c_t *i2c_obj){
    ...
    MSSP_I2C_INTERRUPT_FLAG_CLEAR();
    MSSP_I2C_BUS_COL_FLAG_CLEAR();
    I2C_DefaultInterruptHandler_InterruptHandler = i2c_obj->I2C_DefaultInterruptHandler;
...
/* Interrupt priority Configurations */    
    ...
    if(INTERRUPT_PRIORITY_LOW == i2c_obj->i2c_cfg.mssp_i2c_priority){
        /* Enables all unmasked peripheral interrupts */
        INTERRUPT_LOW_PRIORITY_ENABLE();
        MSSP_I2C_INTERRUPT_PRIORITY_LOW();
    }
    else if(INTERRUPT_PRIORITY_HIGH == i2c_obj->i2c_cfg.mssp_i2c_priority){
    ...
    /*========*/
    if(INTERRUPT_PRIORITY_LOW == i2c_obj->i2c_cfg.mssp_i2c_bc_priority){
        /* Enables all unmasked peripheral interrupts */
        INTERRUPT_LOW_PRIORITY_ENABLE();
        MSSP_I2C_BUS_COL_INTERRUPT_PRIORITY_LOW();
    }
    else if(INTERRUPT_PRIORITY_HIGH == i2c_obj->i2c_cfg.mssp_i2c_bc_priority){
        /* Enables all high-priority interrupts */
        INTERRUPT_HIGH_PRIORITY_ENABLE();
